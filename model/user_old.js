...
UserSchema.pre("save", function(next)                                          //nao usamos arrow function porque da problema quando usamos o "this", chamamos a funcao next;
{
    let user = this;

    if(!user.isModified("password")) return next();                             //se o usuario !nao foi modificado no campo "password", ele da um next(segue em frente);
    bcrypt.hash(user.password, 10, (err, encrypted) =>                          //10 saltos de hash;
    {
